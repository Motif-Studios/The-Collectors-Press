...
export default async function StudioLayout({ children }: { children: ReactNode }) {
  ...
  const user = currentUserResult ?? { name: "User", id: "", userType: "normal" };
  const isAdmin = user.userType === "admin";
  const isAuthor = user.userType === "author";

  // Studio is author-only (admin can still access admin workspace paths if needed)
  if (!isAuthor && !isAdmin) {
    redirect("/");
  }
